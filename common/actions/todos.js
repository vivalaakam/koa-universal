...
export function addTodo(text = '', completed = false) {
  return dispatch => apiTodos.create({ text, completed })
    .then((todo) => {
...
export function updateTodo({ id, text = '', completed = false }) {
  return dispatch => apiTodos.update(id, { text, completed })
    .then((todo) => {
...
export function deleteTodo(id) {
  return dispatch => apiTodos.remove(id)
    .then(() => {
...
export function completeAll() {
  return dispatch => apiTodos.completeAll()
    .then((todos) => {
...
export function clearCompleted() {
  return dispatch => apiTodos.clearCompleted()
    .then((todos) => {
      dispatch(listTodoDispatch(todos));
    });
}
